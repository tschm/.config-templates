...
# show_usage prints the script usage, available commands (bump, release), bump-specific options, and example invocations.
show_usage() {
...
# prompt_continue displays a prompt with the given message and exits the script unless the user confirms by entering a response that starts with "y" or "Y".
prompt_continue() {
  local message="$1"
...
# do_bump bumps or sets the project version in pyproject.toml, validates the new version and repository state, and optionally commits the change.
do_bump() {
...
# do_release creates an annotated git tag for the current pyproject.toml version and pushes it to origin, performing safety checks (branch, uncommitted changes, upstream sync, existing tags) and prompting the user before destructive or network actions.
do_release() {
