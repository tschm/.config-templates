...
def git_repo(tmp_path, monkeypatch):
    """
    Create a bare remote Git repository, clone it to a local workspace prepared for tests, and yield the local repository path.
    
    The local repository is populated with:
    - pyproject.toml containing version "0.1.0"
    - an empty uv.lock
    - a mock bin/uv script (content from MOCK_UV_SCRIPT) made executable
    - a copy of the release.sh script at .github/scripts/release.sh made executable
    
    Git user.name and user.email are configured, an initial commit is created, and the initial branch is pushed to the remote. The working directory is changed to the local repository for the duration of the test.
    
    Parameters:
        tmp_path (pathlib.Path): Temporary directory provided by pytest for creating remote and local repositories.
        monkeypatch (pytest.MonkeyPatch): Used to change the current working directory to the local clone.
    
    Returns:
        pathlib.Path: Path to the local cloned repository yielded for use in tests.
    """
